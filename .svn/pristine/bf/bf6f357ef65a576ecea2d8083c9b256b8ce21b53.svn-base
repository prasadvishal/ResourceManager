...
public class
AdminHomeMainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LoggerUtility.log(TAG, "Toolbar has been created");
        ...
        // get the listview
        LoggerUtility.log(TAG, "outside mExpListView");
        LoggerUtility.log(TAG, "after mExpListView");
        // preparing list data
        ...
        mExpListView.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
            ...
            public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
                ...
                LoggerUtility.log(TAG, "You Clicked: " + cPosition);
                LoggerUtility.log(TAG, "its Parent is: " + groupPosition);
                Intent i;
                switch (groupPosition) {
                    case 0:
                        LoggerUtility.toastShort(getApplicationContext(), viewHardwareSummary.get(cPosition).getHardwareType());
                        LoggerUtility.log(TAG, "Inside Switch case of Hardware");
                        LoggerUtility.log(TAG, "Intent COntents: Hardware Name: "+viewHardwareSummary.get(cPosition).getHardwareType()+" Id: "+viewHardwareSummary.get(cPosition).getTypeId());
                        ...
                        break;
                    case 1:
                        LoggerUtility.log(TAG, "Inside Switch case of Software");
                        LoggerUtility.toastShort(getApplicationContext(), viewSoftwareSummary.get(cPosition).getSoftwareName());
                        i = new Intent(getApplicationContext(), SoftwareDetailsActivity.class);
                        LoggerUtility.log(TAG, "Intent created, ready to send data to software details");
                        ...
                        LoggerUtility.toastShort(getApplicationContext(),"" + viewSoftwareSummary.get(cPosition).getSoftwareID());
                        i.putExtra(SOFTWARE_COUNT, viewSoftwareSummary.get(cPosition).getValidCount());
                        LoggerUtility.log(TAG, "Data sent: " + viewSoftwareSummary.get(cPosition).getSoftwareID() + " " + viewSoftwareSummary.get(cPosition).getValidCount() + " ");
                        startActivity(i);
                        overridePendingTransition(R.anim.slide_from_right, R.anim.slide_to_left);
                        break;
                    case 2:
                        LoggerUtility.toastShort(getApplicationContext(), viewSharedResource.get(cPosition).getHardwareType());
                        LoggerUtility.log(TAG, "Inside Switch case of Shared Resource");
                        break;
                    case 3:
                        Intent intentPendingReq = new Intent(getApplicationContext(), RequestSummaryActivity.class);
    ...
    void prepareListWithData() {
        //TODO placeholder progressbar //Done
        ...
        Call<AllResourceSummary> call = callAPIInterface.getResourceSummary(LOGIN_REQUEST_HEADER_VALUE, token);
        LoggerUtility.log(TAG, token);
        LoggerUtility.log(TAG, "response in call object");
        call.enqueue(new Callback<AllResourceSummary>() {
                         @Override
                         public void onResponse(Call<AllResourceSummary> call, Response<AllResourceSummary> response) {
                             //response's body
                             ...
                             try {
                                 ...
                                 if (response.body().getResponse().getCode() == OK) {
                                     ...
                                     for (int i = 0; i < viewHardwareSummary.size(); i++) {
                                         ...
                                         LoggerUtility.log(TAG, viewHardwareSummary.get(i).getHardwareType());
                                     }
                                     for (int i = 0; i < viewSoftwareSummary.size(); i++) {
                                         softwareResources.add(i, viewSoftwareSummary.get(i).getSoftwareName());
                                         LoggerUtility.log(TAG, viewSoftwareSummary.get(i).getSoftwareName()+" "+viewSoftwareSummary.get(i).getSoftwareID());
                                     }
                                     for (int i = 0; i < viewSharedResource.size(); i++) {
                                         sharedResources.add(i, viewSharedResource.get(i).getHardwareType());
                                         LoggerUtility.log(TAG, viewSharedResource.get(i).getHardwareType());
                                     }
                         ...
                         @Override
                         public void onFailure(Call<AllResourceSummary> call, Throwable t) {
                             progressDialog.dismiss();
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case (R.id.fab_new_software):
                LoggerUtility.log(TAG, "inside on click listener, placeholder new software");
                Intent intentSoftware = new Intent(AdminHomeMainActivity.this, AddSoftwareActivity.class);
                startActivity(intentSoftware);
                overridePendingTransition(R.anim.slide_from_right, R.anim.slide_to_left);
                break;

            case (R.id.fab_new_hardware):
                Intent intentHardware = new Intent(AdminHomeMainActivity.this, AddHardwareActivity.class);
